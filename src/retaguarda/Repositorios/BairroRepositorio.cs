...
namespace Retaguarda.Repositorios
{
    public class BairroRepositorio : RepositorioBase<EnderecoBairro>, IBairroRepositorio
    {
        ...
        /// </summary>
        public async Task<(List<EnderecoBairro> Items, int Total)> ListarComMunicipioAsync(string? nomeFilter, int page, int pageSize, string? sortField, string? sortDir, IDictionary<string, string>? filtros = null, int? inativo = null)
        {
            var q = _dbSet.Include(b => b.Municipio).AsQueryable();
            q = AplicarFiltroMultilocatario(q);

            var propAtivo = typeof(EnderecoBairro).GetProperty("Ativo");
