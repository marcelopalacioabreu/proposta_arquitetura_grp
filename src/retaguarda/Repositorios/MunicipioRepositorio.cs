...
namespace Retaguarda.Repositorios
{
    public class MunicipioRepositorio : RepositorioBase<EnderecoMunicipio>, IMunicipioRepositorio
    {
        ...
        /// </summary>
        public async Task<(List<EnderecoMunicipio> Items, int Total)> ListarComUfAsync(string? nomeFilter, int page, int pageSize, string? sortField, string? sortDir, IDictionary<string, string>? filtros = null, int? inativo = null)
        {
            var q = _dbSet.Include(m => m.Uf).AsQueryable();
            q = AplicarFiltroMultilocatario(q);

            var propAtivo = typeof(EnderecoMunicipio).GetProperty("Ativo");
