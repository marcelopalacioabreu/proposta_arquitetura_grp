...
namespace Retaguarda.Api.Controllers
{
    ...
    public class DocumentoTipoController : BaseController
    {
        ...
        public DocumentoTipoController(Retaguarda.Persistencia.IApplicationDbContext db)
        {
            _db = db;
        }
        ...
        {
            var query = _db.DocumentoTipos.AsQueryable();
        ...
        {
            var e = _db.DocumentoTipos.Find(id);
            if (e == null) return NotFoundError("Registro não encontrado");
            return OkData(e);
        }

        [HttpPost]
        public IActionResult Create([FromBody] DocumentoTipo dto)
        {
            _db.DocumentoTipos.Add(dto);
            _db.SaveChanges();
            return CreatedDataAtAction(nameof(Get), new { id = dto.Id }, dto, "Criado com sucesso");
        }

        [HttpPut("{id}")]
        public IActionResult Update(long id, [FromBody] DocumentoTipo dto)
        {
            var e = _db.DocumentoTipos.Find(id);
            if (e == null) return NotFoundError("Registro não encontrado");
