...
namespace Retaguarda.Api.Controllers
{
    ...
    public class TipoUnidadeController : BaseController
    {
        ...
        public TipoUnidadeController(Retaguarda.Persistencia.IApplicationDbContext db)
        {
            _db = db;
        }
        ...
        {
            var query = _db.TiposUnidade.AsQueryable();
        ...
        {
            var e = _db.TiposUnidade.Find(id);
            if (e == null) return NotFoundError("Registro não encontrado");
            return OkData(e);
        }

        [HttpPost]
        public IActionResult Create([FromBody] TipoUnidade dto)
        {
            _db.TiposUnidade.Add(dto);
            _db.SaveChanges();
            return CreatedDataAtAction(nameof(Get), new { id = dto.Id }, dto, "Criado com sucesso");
        }

        [HttpPut("{id}")]
        public IActionResult Update(long id, [FromBody] TipoUnidade dto)
        {
            var e = _db.TiposUnidade.Find(id);
            if (e == null) return NotFoundError("Registro não encontrado");
