...
namespace Retaguarda.Api.Controllers
{
    ...
    public class NaturezaJuridicaController : BaseController
    {
        ...
        public NaturezaJuridicaController(Retaguarda.Persistencia.IApplicationDbContext db)
        {
            _db = db;
        }
        ...
        {
            var e = _db.NaturezasJuridicas.Find(id);
            if (e == null) return NotFoundError("Registro não encontrado");
            return OkData(e);
        }

        [HttpPost]
        public IActionResult Create([FromBody] NaturezaJuridica dto)
        {
            _db.NaturezasJuridicas.Add(dto);
            _db.SaveChanges();
            return CreatedDataAtAction(nameof(Get), new { id = dto.Id }, dto, "Criado com sucesso");
        }

        [HttpPut("{id}")]
        public IActionResult Update(long id, [FromBody] NaturezaJuridica dto)
        {
            var e = _db.NaturezasJuridicas.Find(id);
            if (e == null) return NotFoundError("Registro não encontrado");
