...
namespace Retaguarda.Api.Controllers
{
    ...
    public class TipoContatoController : BaseController
    {
        ...
        {
            var query = _db.TiposContato.AsQueryable();
        ...
        {
            var e = _db.TiposContato.Find(id);
            if (e == null) return NotFoundError("Registro não encontrado");
            return OkData(e);
        }

        [HttpPost]
        public IActionResult Create([FromBody] TipoContato dto)
        {
            _db.TiposContato.Add(dto);
            _db.SaveChanges();
            return CreatedDataAtAction(nameof(Get), new { id = dto.Id }, dto, "Criado com sucesso");
        }

        [HttpPut("{id}")]
        public IActionResult Update(long id, [FromBody] TipoContato dto)
        {
            var e = _db.TiposContato.Find(id);
            if (e == null) return NotFoundError("Registro não encontrado");
