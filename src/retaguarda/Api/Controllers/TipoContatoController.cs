...
namespace Retaguarda.Api.Controllers
{
    ...
    {
        private readonly ITipoContatoServico _servico;

        public TipoContatoController(ITipoContatoServico servico)
        {
            _servico = servico;
        }

        [HttpGet]
        [Authorize(Policy = "catalogos.tipoContato.visualizar")]
        ...
        [HttpPost]
        [Authorize(Policy = "catalogos.tipoContato.editar")]
        public IActionResult Create([FromBody] TipoContatoDto dto)
        {
        ...
        [HttpDelete("{id}")]
        [Authorize(Policy = "catalogos.tipoContato.excluir")]
        ...
        [HttpPut("{id}")]
        [Authorize(Policy = "catalogos.tipoContato.editar")]
        public IActionResult Update(long id, [FromBody] TipoContatoDto dto)
        {
            if (!ModelState.IsValid) return BadRequestModelState();
